...
func CreateSessionAndSendMessage(userName string, userQuestion string, modelType string) (string, string, code.Code) {
	...
	if err != nil {
		slog.Error("GetOrCreateAIHelper failed", "error", err)
	...
	if err_ != nil {
		slog.Error("GenerateResponse failed", "error", err_)
		return "", "", code.AIModelFail
	}
...
func StreamMessageToExistingSession(userName string, sessionID string, userQuestion string, modelType string, writer http.ResponseWriter) code.Code {
	...
	cb := func(msg string) {
		// 直接发送数据，不转义
		// SSE 格式：data: <content>\n\n
		slog.Debug("SSE chunk", "len", len(msg))
	...
	if err_ != nil {
		slog.Error("StreamResponse failed", "error", err_)
		return code.AIModelFail
	}

	_, err = writer.Write([]byte("data: [DONE]\n\n"))
	if err != nil {
		slog.Error("SSE write DONE failed", "error", err)
		return code.AIModelFail
	}
	flusher.Flush()
