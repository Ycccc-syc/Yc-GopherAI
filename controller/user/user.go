...
import (
	...
	servuser "Yc-GopherAI/service/user"
...
func Login(c *gin.Context) {
	...
	token, code_ := servuser.Login(req.Username, req.Password)
...
func Register(c *gin.Context) {
	...
	token, code_ := servuser.Register(req.Email, req.Password, req.Captcha)
	if code_ != code.CodeSuccess {
		c.JSON(http.StatusOK, res.CodeOf(code_))
		return
	}
...
func HandleCaptcha(c *gin.Context) {
	...
	//给service层进行处理
	code_ := servuser.SendCaptcha(req.Email)
	if code_ != code.CodeSuccess {
		c.JSON(http.StatusOK, res.CodeOf(code_))
		return
	}
